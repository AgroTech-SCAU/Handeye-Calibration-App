...
def corner_subpix_win(gray, pattern_size, corners=None):
    ...
    if corners is not None:
        ...
        spacings = []
        if pts.shape[1] > 1:
            spacings.append(np.linalg.norm(pts[:, 1:] - pts[:, :-1], axis=2).ravel())
        if pts.shape[0] > 1:
            spacings.append(np.linalg.norm(pts[1:] - pts[:-1], axis=2).ravel())
        if spacings:
...
def validate_chessboard_geometry(corners, pattern_size, min_spacing_ratio=0.45):
    ...
    neighbor = []
    if cols > 1:
        neighbor.append(np.linalg.norm(grid[:, 1:] - grid[:, :-1], axis=2).ravel())
    if rows > 1:
        neighbor.append(np.linalg.norm(grid[1:] - grid[:-1], axis=2).ravel())
...
def load_sample_validity(
    path, max_reprojection_px=0.40, max_corner_rms_px=0.20, max_sync_dt_ms=80.0
):
    """在求解前执行不可被权重掩盖的硬质量门禁
    ...
    for i, sample in enumerate(samples):
        ...
        if (
            corner_rms is not None
            and np.isfinite(float(corner_rms))
        ...
        if (
            sync_dt is not None
            and np.isfinite(float(sync_dt))
            and float(sync_dt) > float(max_sync_dt_ms)
        ):
            reasons.append(f"sync_dt={float(sync_dt):.1f}ms>{max_sync_dt_ms:.1f}ms")
...
def prepare_inputs(b2g_list, t2c_list, mode):
    ...
    for b2g, t2c in zip(b2g_list, t2c_list):
        ...
        robot_pose = b2g if mode == "eye_in_hand" else np.linalg.inv(b2g)
