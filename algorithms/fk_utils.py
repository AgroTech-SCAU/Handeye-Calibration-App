...
def fk_gripper_in_base(joints_rad: Sequence[float]) -> np.ndarray:
    """按 robot_params.yaml 中的 MDH 关节数执行正运动学.
    ...
    if joints.size != expected:
        raise ValueError(
            f"当前 robot_params.yaml 定义 {expected} 自由度，"
            f"但收到 {joints.size} 个关节角"
        )
    if not all(len(values) == expected for values in (
            _MDH_D, _MDH_ALPHA, _MDH_OFFSET, _MDH_SIGN)):
        raise ValueError("robot_params.yaml 的 a/d/alpha/offset/sign 长度不一致")
