...
def diagnose(samples_path):
    ...
    print(
        f"  旋转角分布: mean={np.mean(angles):.1f}°  std={np.std(angles):.1f}°  "
    ...
    print(
        f"  分布: 0-30°:{hist[0]}  30-90°:{hist[1]}  90-150°:{hist[2]}  150-180°:{hist[3]}"
    )
    if hist[3] > 0:
    ...
    print(
        f"  mean={np.mean(dists):.0f}mm  std={np.std(dists):.0f}mm  "
    ...
    # 优先使用 samples.yaml 记录的采集内参；GUI 可把输出放在独立目录，
    # 因此不能总是读取本脚本目录中的旧 camera_intrinsics.yaml
    intrinsics_name = data.get("intrinsics_file", "camera_intrinsics.yaml")
    intrinsics_file = (
        intrinsics_name
        if os.path.isabs(intrinsics_name)
        else os.path.join(
            os.path.dirname(os.path.abspath(samples_path)), intrinsics_name
        )
    )
    ...
    if sq_mm is None:
        ...
        if cam_sq is not None and cam_sq != sq_mm:
            print(
                f"  ⚠️  samples.yaml 中 square_size_mm={sq_mm}，但 camera_intrinsics.yaml 中={cam_sq}，不一致!"
            )
            print(f"     使用 samples.yaml 的值 ({sq_mm}mm) 计算 px/格")
    fx = cam_data["camera_matrix"]["data"][0]
    px_per_sq = fx * sq_mm / dists
    print(
        f"  px/格 (基于 fx={fx:.1f}): mean={np.mean(px_per_sq):.0f}  min={np.min(px_per_sq):.0f}"
    )
    ...
    for axis, name in enumerate(["X", "Y", "Z"]):
        vals = b2g_pos[:, axis]
        print(
            f"  {name}: span={np.max(vals)-np.min(vals):.3f}m  "
    ...
    if s[-1] < 0.05:
        print(
            f"  ⚠️  旋转多样性不足 (最小奇异值={s[-1]:.4f})！机械臂需要不同方向的旋转"
        )
    ...
    if n >= 10:
        ...
        print(
            f"    mean={np.mean(pair_errors):.1f}°  median={np.median(pair_errors):.1f}°  "
        ...
        try:
            ...
            t_rms = (
                float(
                    np.sqrt(
                        np.mean(
                            np.linalg.norm(trans - np.mean(trans, axis=0), axis=1) ** 2
                        )
                    )
                )
                * 1000
            ...
            r_rms = float(
                np.sqrt(
                    np.mean(
                        [
                            rotation_angle_deg(mean_R.T @ c[:3, :3]) ** 2
                            for c in constants
                        ]
                    )
                )
    ...
    if hist[3] > n * 0.05:
        issues.append(
            f"大量样本 ({hist[3]}) 中 target_to_camera 旋转 >150° → 系统性翻转"
        )
    if low_res > n * 0.1:
    ...
    if not issues:
        print(f" [WARN] 未发现明显异常，但误差仍大；请检查:")
